...
def decode(path, sr, channels, filt=None):
    ...
    if filt:
        cmd += ["-af", filt]
    cmd += ["-map", "0:a:0", "-ac", str(channels), "-ar", str(sr),
...
def main():
    ...
    truth = decode(args.source, sr, 2,
                   "atrim=start_sample=%d:end_sample=%d" % (s0, s1))
    ...
    print("content  : %d of %d samples differ (%.5f%%), max delta %d"
    ...
    print("         : differences span %.1f ms .. %.1f ms from the start"
          % (first_ms, last_ms))
    ...
    print("timing   : %+d samples (%+.3f ms) past the first %.0f ms, confidence %.3f"
    ...
    if last_ms <= 150.0:
        print("\nPASS - sample exact in time; first %.1f ms differ "
              "(bit-reservoir warm-up)" % last_ms)
    else:
        print("\nPASS - sample exact in time; content differs throughout "
              "(expected when re-encoded)")
    return 0
